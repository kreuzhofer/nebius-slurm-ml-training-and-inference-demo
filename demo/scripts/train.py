...
def main():
    ...
    # Use local SSD for checkpoints during training (much faster than shared NFS).
    # The final model gets copied back to the shared output_dir after training.
    ...
    # Caching is disabled (datasets.disable_caching() at module level) so each
    # rank processes the dataset independently in memory — no shared NFS cache
    # files, no cross-node race conditions.
    ...
    # The Trainer already saved a full checkpoint (with model safetensors) to local
    # SSD during training. We copy the model files from the last checkpoint to the
    # shared NFS output_dir so evaluate/serve scripts can find them.
    # NOTE: We intentionally skip trainer.save_model() here because the FSDP
    # FULL_STATE_DICT all-gather after training exceeds the torchrun rendezvous
    # timeout (300s) and kills the job. The checkpoint already has the full model.
    if trainer.is_world_process_zero():
        import glob
        checkpoints = sorted(glob.glob(os.path.join(local_output_dir, "checkpoint-*")))
        if checkpoints:
            last_ckpt = checkpoints[-1]
            print(f"Copying model from {last_ckpt} to {output_dir} ...")
            os.makedirs(output_dir, exist_ok=True)
            for fname in os.listdir(last_ckpt):
                src = os.path.join(last_ckpt, fname)
                if os.path.isfile(src) and not fname.startswith(("rng_state", "optimizer", "pytorch_model_fsdp", "training_args")):
